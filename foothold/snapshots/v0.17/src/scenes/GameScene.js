...
export class GameScene extends Phaser.Scene {
  ...
  // The single-tile slice of refresh()'s fill/ripple logic, reused by the wipe's staggered
  // timers so each tile settles into exactly the same state refresh() would have given it.
  applyShoalVisual(r, c) {
    const t = this.tiles[r][c];
    const v = this.views[r][c];
    if (this.isSubmerged(t)) v.rect.setFillStyle(SHOAL_HIGH_COLOR);
    else if (t.owner === 0) v.rect.setFillStyle(SAND_COLOR);
    else v.rect.setFillStyle(FILL[t.owner]);
    if (v.ripple) v.ripple.setVisible(this.isSubmerged(t));
  }
  ...
  buildShorelineCorners() {
    ...
    const waterNeighborCount = (r, c) => {
      let n = 0;
      if (isWaterTile(r - 1, c)) n++;
      if (isWaterTile(r + 1, c)) n++;
      if (isWaterTile(r, c - 1)) n++;
      if (isWaterTile(r, c + 1)) n++;
      return n;
    };
    ...
    this.forEachTile((t, r, c) => {
      ...
      const water = t.ocean || t.shoal;
      const up = isWaterTile(r - 1, c), down = isWaterTile(r + 1, c);
      const left = isWaterTile(r, c - 1), right = isWaterTile(r, c + 1);
      const flags = water
  ...
  // paint the corner's RxR box in the board background, then redraw the tile's own rounded
  // quarter-disc on top; the circle is tangent to both edges so it never spills past the tile's
  // own bounds. Net effect: a clean rounded corner instead of a sharp square one.
  ...
  drawSandGlitter() {
    ...
    this.forEachTile((t, r, c) => {
      const v = this.views[r][c];
      if (!v.glitterDots || t.owner !== 0 || this.isSubmerged(t)) return;
  ...
  // (on the ocean level) the sand-glitter twinkle.
  update() {
    this.drawRiverFlow();
  ...
  checkWin() {
    ...
    if (aiBaseOwner === 1) {
      ...
      if (totalVictory) { msg = 'Complete Foothold!'; sub = `Ultimate Victory by Round ${winRound}`; }
