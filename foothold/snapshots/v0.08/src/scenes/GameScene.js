...
export class GameScene extends Phaser.Scene {
  ...
  computeLayout() {
    ...
    if (!this.isWide) {
      ...
      this.roundY = 140;                // turn/round text Y (tiles score sits +56 below)
    ...
    this.legendTop = this.roundY + 104;                    // below the round + tiles lines
  ...
  buildHud() {
    ...
    // Centered on the rail: the full canvas in portrait, the right column in wide mode.
    const midX = railX + railW / 2;
    this.turnText = this.add.text(midX, this.roundY, '', {
      fontFamily: HEAD, fontSize: '34px', fontStyle: 'bold', color: '#e7e9f0',
    }).setOrigin(0.5, 0);
    this.tilesText = this.add.text(midX, this.roundY + 56, '', {
  ...
  buildControls() {
    ...
    this.add.text(newCx, btnY, 'New\nGame', {
      fontFamily: HEAD, fontSize: '24px', fontStyle: 'bold', color: '#e7e9f0', align: 'center',
    }).setOrigin(0.5);
  ...
  refresh() {
    ...
    this.forEachTile((t, r, c) => {
      ...
      if (t.resource) {
        ...
        v.icon.setTexture('ic_home').setDisplaySize(66, 66).setTint(0xffffff).setVisible(true);
      } else {
