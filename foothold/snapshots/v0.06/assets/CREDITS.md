...
- **Board Game Icons** — Kenney (Kenney Vleugels) — https://kenney.nl/assets/board-game-icons — License: CC0 1.0 (public domain, commercial use OK, no attribution required).
  - Used (recolored/tinted per resource): `d2` → gold, `resource_lumber` → wood, `resource_wood` → stone (brick-like log tinted grey; pack has no stone icon), `award` → special node, `structure_house` → home tile, `card_lift` → upgrade marker, `structure_watchtower` → title-screen hero art (tinted gold; game-ready copy at `assets/watchtower.png`).
