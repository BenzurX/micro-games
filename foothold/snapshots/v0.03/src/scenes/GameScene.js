...
export class GameScene extends Phaser.Scene {
  ...
  buildControls() {
    ...
    const legend = [
      { color: ACTION_COLOR.claim,   label: 'Claim with gold',   desc: 'empty tiles',              res: 'gold',  amt: CLAIM_COST },
      { color: ACTION_COLOR.build,   label: 'Build with wood',   desc: 'resource tiles',           res: 'wood',  amt: BUILD_COST },
      { color: ACTION_COLOR.fortify, label: 'Fortify with stone', desc: 'your resource nodes',     res: 'stone', amt: FORTIFY_COST },
      { color: ACTION_COLOR.siege,   label: 'Siege with gold',   desc: 'enemy tiles (x2 fortified)', res: 'gold',  amt: SIEGE_COST },
    ];
    ...
    legend.forEach((item, i) => {
      ...
      this.add.text(textX, yMid + 1, item.label, {
        fontFamily: font, fontSize: '20px', fontStyle: 'bold', color: '#e7e9f0',
      }).setOrigin(0, 1);
  ...
  onTileTap(r, c) {
    ...
    if (t.owner === 0 && adj) {
      ...
      // Enemy tile: Siege (gold). Fortified tiles cost 2x; the enemy home is very steep.
      if (res.gold >= this.siegeCost(t)) this.applyMove(1, { type: 'siege', r, c });
      else return;
  ...
  // ---------------------------------------------------------------- helpers
  // Gold cost to siege an enemy tile: home is deliberately steep, walls double the base.
  siegeCost(t) {
  ...
  gainFor(action, t) {
    const GREEN = '#5ac16f';
    if (action === 'fortify') return { icon: 'fortify', tint: ACTION_COLOR.fortify, big: '×2', unit: 'vs siege', color: '#caa9ff' };
