...
export class Sfx {
  ...
  play(name, opts = {}) {
    ...
    switch (name) {
      ...
      case 'tideHigh': { // nautical two-strike ship's bell announcing high tide taking the
        // shoreline - a metal bell, not a glass ting: inharmonic partials (like clang()'s anvil,
        // but pitched up and left ringing longer) plus a detuned pair on the upper partials for
        // shimmer/beating. Each strike sustains well past the strike itself, and two quiet,
        // longer-decaying echoes trail off after, like the tone bouncing back over open water.
        const base = 1320 * p; // high, bright, but low enough to still read as metal not glass
        const partials = [
          { r: 1.00, g: 0.40, d: 1.1 },
          { r: 2.42, g: 0.24, d: 0.85, dt: 3 },
          { r: 3.88, g: 0.15, d: 0.6, dt: 5 },
          { r: 5.64, g: 0.09, d: 0.4, dt: 6 },
        ];
        const strike = (delay, level) => {
          partials.forEach(({ r, g, d, dt }) => {
            const offsets = dt ? [-dt, dt] : [0];
            offsets.forEach((off) => this.tone({
              freq: base * r + off, dur: d, type: r === 1 ? 'sine' : 'triangle',
              gain: (g * level) / offsets.length, delay, attack: 0.003,
            }));
          });
        };
        strike(0, 1);        // first strike, full ring
        strike(0.32, 0.8);   // second strike (the "double bell"), slightly softer
        // Ring echo: quiet, longer repeats of the fundamental + first overtone, fading further
        // out each time, so the bell trails off instead of cutting dead after the two strikes.
        this.tone({ freq: base, dur: 1.4, type: 'sine', gain: 0.09, delay: 0.62, attack: 0.015 });
        this.tone({ freq: base * 2.42, dur: 0.9, type: 'sine', gain: 0.05, delay: 0.62, attack: 0.015 });
        this.tone({ freq: base, dur: 1.6, type: 'sine', gain: 0.04, delay: 1.05, attack: 0.02 });
        break;
      }
