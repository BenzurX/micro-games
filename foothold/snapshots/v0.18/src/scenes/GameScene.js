...
export class GameScene extends Phaser.Scene {
  ...
  buildOceanWater() {
    ...
    // Fringe-ripple phase offset: same diagonal walk as the ocean cascade, plus a little
    // jitter so equal diagonals don't lock together exactly.
    const stagger = (r, c) => ((r + c) % 6) * 160 + Phaser.Math.Between(0, 120);

    const foams = [];
    this.forEachTile((t, r, c) => {
      const v = this.views[r][c];
      if (t.ocean) {
        // Two soft light bands per tile, drifting in opposite directions at different speeds
        // (TileSprites scroll their texture inside a fixed frame, so nothing needs a mask).
        // The cascade: every tile starts (r + c) beats INTO its cycle - seek() jumps the tween
        // mid-flight, the per-tile equivalent of the stage mockup's negative animation-delay,
        // so no tile ever sits frozen waiting for a start cue.
        const cascade = (r + c) * 450;
        // Normal alpha blending (no ADD): the stage composites the foam color straight over
        // the water fill, and additive blending washes the crest out into an undefined glow.
        v.swellSprites = [];
        const swell = (key, fromX, toX, dur) => {
          // Masked to this tile's own rounded-corner shape (same as the shoal ripple below) so
          // the shimmer's square TileSprite frame gets cut away with the rest of a rounded ocean
          // corner instead of poking a hard square edge past the curve. Ocean tile rounding is
          // tide-dependent (computeCornerPlan), so unlike the shoal ripple's mask (built
          // once, since a shoal tile's OWN corners never change) this one is rebuilt every tide
          // phase change - see updateOceanSwellMasks(), called from advanceTide().
        ...
        // readable: finer ripple stripes on a constant linear drift (no roll-back), dimmer, and
        // on its own per-tile clock. refresh() shows it only during the submerged round.
        // Masked to this tile's own rounded-corner shape (tileGeometryMask) so the flooded-fringe
        // ripple never pokes past the shoreline curve once the tide is fully in - without the
        // mask, the tileSprite's square frame overwrites the rounded notch that sits below it.
        ...
        this.tweens.add({
          ...
          duration: 2400 + ((r * 5 + c * 3) % 5) * 260, delay: stagger(r, c), repeat: -1,
        });
        v.ripple = rip;
        ...
        // Shining-sand glitter (stage/sand-glitter.html option B): a handful of specks per tile,
        // each fading in/out on its own slow, staggered cycle - only ever a few "lit" at once.
        // Positions are inset well clear of the tile edge so they never need the rounded-corner
        // mask; drawSandGlitter() (called from update()) only shows them while the tile is dry
        // and unclaimed, i.e. exactly the SAND_COLOR state this is meant to accent.
    ...
    // One shared alpha-pulse across all foam warnings (the pulse IS the warning; it doesn't
    // need per-tile phase, and a single tween is cheaper than twenty).
    if (foams.length) {
      this.tweens.add({
        targets: foams, alpha: { from: 0.3, to: 0.85 },
  ...
  // Is this tile "water" for shoreline-rounding purposes RIGHT NOW - ocean is always water; a
  // shoal tile counts as water unless it's dry (low tide, tidePhaseIndex >= 2), in which case it
  // reads as land/beach instead. Used for BOTH sides of every boundary (land tiles' concave
  // corners and water tiles' convex corners) via computeCornerPlan() below - using one shared,
  // live classification for both sides is what keeps them from ever disagreeing about where the
  // boundary is. An earlier version had land tiles use a permanent "shoal always counts as
  // water" rule while ocean tiles used this live rule, so at low tide the two systems could each
  // carve their own rounded corner at the very same grid vertex - two overlapping circles instead
  // of one clean curve, showing up as a small stray colored dot right at the shoreline.
  isWaterNow(r, c) {
    const t = this.tiles[r] && this.tiles[r][c];
    if (!t) return false;
    if (t.ocean) return true;
    if (t.shoal) return !(this.tidePhaseIndex >= 2);
    return false;
  }
  ...
  // dry shoal. Recomputed on every tide-phase change (advanceTide()), not just once at build time,
  // since which shoal tiles read as "water" vs "beach" changes over the course of a game.
  computeCornerPlan() {
    const waterNeighborCount = (r, c) => {
      let n = 0;
      if (this.isWaterNow(r - 1, c)) n++;
      if (this.isWaterNow(r + 1, c)) n++;
      if (this.isWaterNow(r, c - 1)) n++;
      if (this.isWaterNow(r, c + 1)) n++;
      return n;
    };
    ...
    this.forEachTile((t, r, c) => {
      ...
      const water = this.isWaterNow(r, c);
      const up = this.isWaterNow(r - 1, c), down = this.isWaterNow(r + 1, c);
      const left = this.isWaterNow(r, c - 1), right = this.isWaterNow(r, c + 1);
      const flags = water
