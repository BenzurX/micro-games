...
import { CRT } from '../lib/CrtPipeline.js';
...
const RESOURCE_CAP = 100;       // hard ceiling on every stockpile — income past this is lost
...
export class GameScene extends Phaser.Scene {
  ...
  // Attach the CRT post-processing shader to the main camera. Guarded two ways:
  // the CRT.enabled flag (easy off switch), and the renderer type — the shader
  // only exists on WebGL, so a Canvas fallback simply runs the game unfiltered.
  applyCrt() {
  ...
  buildControls() {
    ...
    const legend = [
      ...
      { color: ACTION_COLOR.upgrade, label: 'Upgrade resource nodes',  desc: 'Double that tile\'s output',   res: 'stone', amt: UPGRADE_COST },
      { color: ACTION_COLOR.siege,   label: 'Siege enemy tiles',       desc: 'Capture — nodes also cost wood', res: 'gold',  amt: SIEGE_COST },
    ];
    ...
    // --- End Turn, anchored to the very bottom of the screen ---
    const btnY = this.buttonY; // computed in computeLayout so it hugs the bottom on tall phones
  ...
  showTooltip(r, c) {
    ...
    const ICON_W = 17, GAP_IN = 3, GAP_PAIR = 8;
    ...
    const costParts = costs.map(({ res, val }) => ({
      ...
      num: this.add.text(0, 0, `${val}`, { fontFamily: font, fontSize: '15px', fontStyle: 'bold', color: DARK }).setOrigin(0, 0.5),
    }));
