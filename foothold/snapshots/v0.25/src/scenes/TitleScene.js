...
// Display serif (Grenze) for the medieval feel - used ONLY on the title, tagline, and buttons.
// Body/label text keeps FONT so it stays crisp under the CRT filter. Georgia/serif fall back
// if the webfont hasn't loaded (main.js waits for it, so this is just a safety net).
...
export class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    applyDlss(this); // seed the DLSS gag (sparkles + god rays) if it was left on
    ...
    // Live canvas size (720-wide portrait on phones, ~1200-1700 x 900 in wide/desktop mode).
    // Every position below derives from W/H fractions, so one layout serves both modes.
    ...
    this.add.text(cx, Math.round(H * 0.50), 'FOOTHOLD', {
      fontFamily: HEAD, fontSize: '84px', fontStyle: 'bold', color: INK,
    }).setOrigin(0.5);
    ...
    const wobjs = words.map((w) => this.add.text(0, tagY, w.t, {
      fontFamily: HEAD, fontSize: '32px', fontStyle: 'bold', color: w.c,
    ...
    const start = this.button(cx, startY, 460, 104, 'Start', ACCENT, '#ffffff', 60, () => {
      sfx.play('newgame');
      this.scene.start('LevelSelectScene');
    });
    // A gentle breathing pulse on the primary CTA to pull the eye. Paused on hover so it stops
    // fighting the button's own hover-grow (both drive the label's scale every frame otherwise).
    const pulse = this.tweens.add({ targets: start.objs, scale: { from: 1, to: 1.03 }, duration: 1400, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
    start.hit.on('pointerover', () => pulse.pause());
    start.hit.on('pointerout', () => pulse.resume());

    this.button(cx, startY + 128, 460, 72, 'How to Play', 0x323850, INK, 33, () => this.tutorial.show());
    // Jumps straight into the guided tutorial board, skipping LevelSelectScene entirely.
    this.button(cx, startY + 210, 460, 72, 'Play Tutorial', 0x2a2e40, INK, 33, () => {
      sfx.play('newgame');
  ...
  button(cx, y, w, h, labelText, color, textColor, fontSize, onTap) {
    ...
    g.setPosition(cx, y);
    const label = this.add.text(cx, y, labelText, { fontFamily: HEAD, fontSize: `${fontSize}px`, fontStyle: 'bold', color: textColor }).setOrigin(0.5);
