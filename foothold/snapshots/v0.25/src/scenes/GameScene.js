...
// `let`, not `const`: GameScene is a reused Phaser instance (New Game / Play Again restart it
// rather than recreating it), so the tutorial's 4x4 board and the normal 6x9 board must both be
// reachable from the same module state - reassigned in init() per level, same pattern as TILE.
let GRID_W = 6;                 // columns (portrait board)
let GRID_H = 9;                 // rows
const TUTORIAL_SIZE = 4;        // fixed square board for the guided tutorial

// Guided, one-tile-at-a-time script for the fixed 4x4 tutorial board (see placeTutorialNodes).
// Each guarded step names the exact next tile + action, so actionFor()/onTileTap() can lock
// the board down to only that move. After the last guarded step the player is free to Siege
// the AI's remaining tiles/castle in any order.
//
// target/action/need are game logic, not copy - they stay here in code, in step order. The
// on-screen TEXT for each step lives in assets/tutorial-messages.md instead (loaded at runtime,
// see preload() and parseTutorialMd() below), matched to this array by position (the file's
// "## Step 1" maps to index 0, "## Step 2" to index 1, etc.) so Ben can hand-edit the wording
// without touching code. DEFAULT_TUTORIAL_MSGS/DEFAULT_TUTORIAL_* below are only a fallback if
// that file is missing or a section fails to parse.
const TUTORIAL_STEPS = [
  { r: 3, c: 2, action: 'build', need: 'wood' },
  { r: 2, c: 3, action: 'build', need: 'wood' },
  { r: 2, c: 2, action: 'build', need: 'wood' },
  { r: 2, c: 2, action: 'upgrade', need: 'stone' },
  { r: 2, c: 1, action: 'upgrade', need: 'stone' },
  { r: 1, c: 1, action: 'claim', need: 'gold' },
  { r: 0, c: 1, action: 'siege', need: 'gold' },
];
const DEFAULT_TUTORIAL_MSGS = [
  "Tap the <wood> tile to Build it. Build claims a tile that already has a resource node - it costs <wood>, and the tile pays you <wood> income every turn after.",
  "Build the <stone> tile too. <stone> has one job: it funds Upgrade.",
  "Build the <gold> tile next. <gold> funds your aggressive moves - Expand and Siege both spend <gold>.",
  "Now Upgrade that same <gold> tile - spend <stone> to double its income. Works once on any owned resource node, never your home.",
  "Build the <special> tile. One node, a small amount of all three resources every turn - flexible, never a big multiplier.",
  "Tap the empty tile to Expand. Expand grows your territory with no resource under it - just <gold>, no <wood>.",
  "Now <siege> the enemy <gold> tile. Siege takes an enemy resource node for **10** <gold> plus **5** <wood> to re-develop it - double the cost of Build, since you're taking it by force.",
];
const DEFAULT_TUTORIAL_FREE_MSG =
  "The enemy is in reach now. Siege takes any adjacent enemy tile - <gold> to take it, plus <wood> if it holds a resource, since you're re-developing it damaged. Take their tiles in any order.";
const DEFAULT_TUTORIAL_CASTLE_MSG =
  "Their castle is exposed. Siege it to win the tutorial!";
const DEFAULT_TUTORIAL_WAIT_MSG =
  "Not enough <RES> yet for that move. Tap End Turn to collect this turn's income, then come back and try again.";
const DEFAULT_TUTORIAL_INTRO_MSG =
  "**Welcome to Foothold!** Expand your territory, capture resources, and take the <enemy-castle> to win.";

// Parses assets/tutorial-messages.md into { msgs: string[], free, castle, wait, intro }.
// See that file's own header comment for the editing rules (heading names, <res>/<RES> tokens).
// Only extracts TEXT - a "## Step N" heading's message goes into msgs[N-1]; target/action/need
// stay in TUTORIAL_STEPS above and are not read from the file. Any heading it doesn't recognize
// is skipped; missing pieces fall back to the DEFAULT_* values in the caller.
function parseTutorialMd(raw) {
  ...
  for (const sec of sections) {
    ...
    const bodyLines = lines.slice(1).filter((l) => !l.trim().startsWith('<!--') && !l.trim().startsWith('//'));
    const text = bodyLines.join(' ').replace(/\s+/g, ' ').trim();
    const m = heading.match(/^Step (\d+)/i);
    if (m) {
      msgs[Number(m[1]) - 1] = text;
    } else {
      named[heading] = text;
    }
  }
  return {
    msgs,
    free: named['Free Roam'] || '',
    castle: named['Castle Ready'] || '',
    wait: named['Waiting'] || '',
    intro: named['Intro'] || '',
  };
}
...
// Home tile's watchtower icon sits a touch high against its flag - nudge both down together
// (icon in refresh(), flag pole in drawFlags()) so the pair balances vertically on the tile.
const HOME_ICON_Y_NUDGE = 4;
...
export class GameScene extends Phaser.Scene {
  ...
  // Which map to generate. Passed from LevelSelectScene (or the "Tutorial" button in the How to
  // Play overlay) as scene.start('GameScene', {level}). Defaults to 'river' (the original v1
  // map) if launched without data - e.g. a direct scene key start during development.
  init(data) {
    const lvl = data && data.level;
    this.level = (lvl === 'ocean' || lvl === 'tutorial') ? lvl : 'river';
    if (this.level === 'tutorial') { GRID_W = TUTORIAL_SIZE; GRID_H = TUTORIAL_SIZE; }
    else { GRID_W = 6; GRID_H = 9; }
  }

  // Tutorial copy lives in assets/tutorial-messages.md, not code - fetched here (Phaser's
  // loader, not a manual fetch()) so it's ready in this.cache.text by the time create() runs.
  // Only loaded for the tutorial level; the other two levels have no use for it.
  preload() {
    if (this.level === 'tutorial') this.load.text('tutorialMd', 'assets/tutorial-messages.md');
  }
  ...
  create() {
    ...
    // Parse the tutorial's .md copy (see preload() above) - falls back to the DEFAULT_TUTORIAL_*
    // constants per-field if the file is missing or a section didn't parse, so a typo in the .md
    // degrades gracefully instead of breaking the tutorial.
    if (this.level === 'tutorial') {
      const raw = this.cache.text.get('tutorialMd');
      const parsed = raw ? parseTutorialMd(raw) : null;
      this.tutorialSteps = TUTORIAL_STEPS.map((step, i) => ({
        ...step,
        msg: (parsed && parsed.msgs[i]) || DEFAULT_TUTORIAL_MSGS[i],
      }));
      this.tutorialFreeMsg = (parsed && parsed.free) || DEFAULT_TUTORIAL_FREE_MSG;
      this.tutorialCastleMsg = (parsed && parsed.castle) || DEFAULT_TUTORIAL_CASTLE_MSG;
      this.tutorialWaitMsg = (parsed && parsed.wait) || DEFAULT_TUTORIAL_WAIT_MSG;
      this.tutorialIntroMsg = (parsed && parsed.intro) || DEFAULT_TUTORIAL_INTRO_MSG;
    }
    ...
    // Tutorial board has no water at all - skip both the river ribbon and ocean tiles, but
    // still seed bridgeGfx/bridges (empty) so refresh()'s unconditional drawBridges() call has
    // something safe to iterate (same reason buildOceanWater() seeds them - see its comment).
    if (this.level === 'ocean') this.buildOceanWater();
    else if (this.level === 'tutorial') {
      this.bridges = [];
      this.bridgeGfx = this.add.graphics();
    } else this.buildRiver();
    attachTileEditor(this, { tidePhases: TIDE_PHASES }); // hidden dev overlay; see tileEditor.js
    this.buildHud();
    this.buildControls();
    if (this.level === 'tutorial') this.buildTutorialBox();
    ...
    if (this.level === 'tutorial') this.showTutorialIntro();
  }
  ...
  computeLayout() {
    ...
    this.isWide = W > H;
    const isTutorial = this.level === 'tutorial';
    ...
    if (!this.isWide) {
      ...
      this.legendTop = H - (legRowsH + gapLegBtn + btnH + bottomMargin); // top edge of the first legend row
      this.buttonY = H - bottomMargin - btnH / 2;       // center Y of the End Turn button

      // Tutorial only: a full-width message box between the board and the legend, 50% taller
      // than the legend block - the board shrinks a little to make room for it.
      const gapBoardMsg = 12, gapMsgLeg = 12;
      this.msgBoxH = isTutorial ? Math.round(legRowsH * 1.393) : 0;
      this.msgBoxTop = isTutorial ? this.legendTop - gapMsgLeg - this.msgBoxH : null;
      const boardBottom = isTutorial ? this.msgBoxTop - gapBoardMsg : this.legendTop;

      // Grow the tiles to fill the phone: as wide as the canvas allows (20px side margins)
      // and as tall as the HUD-to-board-bottom region allows (24px of breathing room), whichever
      ...
      TILE = Math.max(BASE_TILE, Math.floor(Math.min(
        (W - sidePad * 2) / GRID_W,
        (boardBottom - hudBottom - 24) / GRID_H,
      )));
      BOARD_W = GRID_W * TILE;
      BOARD_H = GRID_H * TILE;
      ...
      const midTop = hudBottom, midBot = boardBottom;
      ...
      if (isTutorial) { this.msgBoxX = this.rail.x; this.msgBoxW = this.rail.w; }
      return;
    }
    ...
    const legRows = 4, legRowsH = legRows * this.legendRowH + (legRows - 1) * 8;

    // Tutorial (desktop): the message box sits BELOW the board, so the board can't just be
    // centered on its own height (that pushes the board+msgbox block past the bottom of the
    // screen, leaving dead space up top and cutting the box off below). Center the whole
    // board+gap+msgbox block instead.
    const msgGap = 20;
    this.msgBoxH = isTutorial ? Math.round(legRowsH * 1.466) : 0;
    const contentH = BOARD_H + (isTutorial ? msgGap + this.msgBoxH : 0);
    this.originY = Math.round((H - contentH) / 2);         // board (+ msgbox) vertically centered
    ...
    // Buttons normally bottom-align with the board, but on a short board (tutorial's 4x4) that
    // would land them ON TOP of the legend below them in the rail - never let them sit higher
    // than the legend's own bottom edge + a gap.
    this.buttonY = Math.max(this.originY + BOARD_H - 48, this.legendTop + legRowsH + 40 + 48);

    if (isTutorial) {
      this.msgBoxTop = this.originY + BOARD_H + msgGap;
      this.msgBoxX = this.originX;
      this.msgBoxW = BOARD_W;
    } else {
      this.msgBoxTop = null;
    }
  }
  ...
  // A soft vertical highlight bar (alpha ramps 0→1→0 left to right, faked with stepped strips
  // since Graphics has no true gradient fill) for the resource-node "glint sweep" idle animation.
  makeGlintTexture() {
    if (this.textures.exists('glintBar')) return;
    const W = 40, H = 64, steps = 20;
    const g = this.make.graphics({ add: false });
    for (let i = 0; i < steps; i++) {
      const f = i / (steps - 1);
      const a = 1 - Math.abs(f - 0.5) * 2;
      g.fillStyle(0xffffff, Math.max(0, a)).fillRect((i / steps) * W, 0, W / steps + 1, H);
    }
    g.generateTexture('glintBar', W, H);
    g.destroy();
  }
  ...
  setupState() {
    ...
    // Tutorial's enemy is a static, unarmed home base - zero starting stockpile (and zero
    // income, see computeIncome()) so it can never afford to claim a tile.
    this.resources = {
      // Tutorial's player starts with just enough wood to build Step 1 immediately, and no
      // stone/gold yet - the guided steps introduce each resource as it becomes relevant.
      1: this.level === 'tutorial' ? { wood: 10, gold: 0, stone: 0 } : { ...START },
      2: this.level === 'tutorial' ? { wood: 0, gold: 0, stone: 0 } : {
    ...
    if (this.level === 'ocean') {
      ...
      this.placeTutorialNodes();
      this.tutorialStep = 0;   // index into this.tutorialSteps; see tutorialAllows()/advanceTutorial()
      this.tutorialFree = false;
      this.tutorialAIClaimed = [false, false]; // has each scripted AI land-grab fired yet? see tutorialAIClaim()
    } else {
      this.generateRiver();
      this.placeNodes();
    }
  ...
  // Fixed 4x4 layout for the guided tutorial (no procedural gen, no river/ocean) - every node
  // is scripted so a future step-by-step walkthrough can tell the player exactly which tile to
  // tap next. 0-indexed [row][col]; homes are already placed above at (0,0) AI / (3,3) player.
  placeTutorialNodes() {
    // 'special' sits at (2,1), not (2,0) - the guided script (TUTORIAL_STEPS) builds it right
    // after the gold tile at (2,2) (adjacent), then Expands into (1,1) right after THAT (also
    // adjacent, via (2,1)) - every guarded step must be orthogonally adjacent to territory the
    // player already owns by that point, or the tap silently fails (see isAdjacentTo/onTileTap).
    const layout = [
      [null, 'gold', null, 'special'],
      [null, null, null, null],
      [null, 'special', 'gold', 'stone'],
      ['wood', null, 'wood', null],
    ];
    for (let r = 0; r < GRID_H; r++) {
      for (let c = 0; c < GRID_W; c++) {
        if (layout[r][c]) this.tiles[r][c].resource = layout[r][c];
      }
    }
    // (1,0) and (0,1) start neutral - the AI scripts a claim on each once the guided steps make
    // them relevant (see tutorialAIClaim(), called from runAITurn()), instead of owning them from
    // turn one. (1,1) stays neutral for good - it's the guided Expand target (step 6); (0,1) holds
    // the enemy gold node - the guided Siege target (step 7), reachable once (1,1) is owned.
  }

  // Is (r,c) a legal tap right now under the tutorial's guardrails? Guarded steps allow only
  // the one scripted tile; once this.tutorialSteps is exhausted the board opens up fully.
  tutorialAllows(r, c) {
    if (this.tutorialFree) return true;
    const step = this.tutorialSteps[this.tutorialStep];
    return !!step && step.r === r && step.c === c;
  }

  // Called right after a tutorial move lands (see onTileTap). Advances the guarded script on a
  // match and flips into free-roam once it runs out. Message text itself is NOT set here - see
  // computeTutorialMessage(), called fresh from refresh() every render, so it always reflects
  // current affordability (e.g. the "out of resources" nudge) instead of going stale.
  advanceTutorial(move) {
    if (this.tutorialFree) return;
    const step = this.tutorialSteps[this.tutorialStep];
    if (step && step.r === move.r && step.c === move.c && step.action === move.type) {
      this.tutorialStep += 1;
      if (this.tutorialStep >= this.tutorialSteps.length) this.tutorialFree = true;
    }
  }

  // What should the message box say RIGHT NOW, given tutorialStep/tutorialFree and the board's
  // current affordability? Re-derived every refresh() (see the tutorial hook near its end) so a
  // guarded step that isn't affordable yet automatically shows the "End Turn" nudge instead of
  // silently doing nothing when tapped, and reverts to the normal step text once affordable again.
  // Also sets this.tutorialWaiting - true whenever the shown message is the "go tap End Turn"
  // nudge (only during a guided Step), so callers can shine the End Turn button to draw the
  // eye to it, and stop as soon as the player has a move again.
  computeTutorialMessage() {
    this.tutorialWaiting = false;
    if (!this.tutorialFree) {
      const step = this.tutorialSteps[this.tutorialStep];
      if (!step) return this.tutorialCastleMsg;
      if (this.actionFor(step.r, step.c)) return step.msg;
      this.tutorialWaiting = true;
      // An 'upgrade' step targeting a still-neutral tile means build-then-upgrade in one step
      // (see the special-node step) - the player needs wood first, stone only once they own it.
      const need = (step.action === 'upgrade' && this.tiles[step.r][step.c].owner !== 1) ? 'wood' : step.need;
      return this.tutorialWaitMsg.replace('<RES>', `<${need}>`);
    }
    let aiTiles = 0;
    this.forEachTile((t) => { if (t.owner === 2) aiTiles += 1; });
    // Only the enemy home left: stay on the Free Roam message (no Waiting nudge) until the
    // home siege becomes affordable, then switch straight to Castle Ready.
    if (aiTiles === 1) {
      if (this.actionFor(this.homeAI.r, this.homeAI.c)) return this.tutorialCastleMsg;
      return this.tutorialFreeMsg;
    }
    let anyMove = false;
    this.forEachTile((t, r, c) => { if (this.actionFor(r, c)) anyMove = true; });
    if (anyMove) return this.tutorialFreeMsg;
    this.tutorialWaiting = true;
    return this.tutorialWaitMsg.replace('<RES>', '<gold>');
  }

  // Renders the current tutorial message AND syncs the End Turn shine + board-tile glint to it -
  // the single call site both buildTutorialBox() and refresh() should use, so all three never
  // drift out of sync.
  refreshTutorialMessage() {
    const msg = this.computeTutorialMessage();
    this.renderTutorialMessage(msg);
    if (this.tutorialWaiting) this.startEndBtnShine();
    else this.stopEndBtnShine();
    this.updateTutorialTileTarget();
  }

  // Which tile (if any) should glint right now to draw the eye to the guided move: the current
  // guarded step's tile while one is active AND affordable right now, the enemy home tile once
  // free-roam has whittled the AI down to just its castle and it's affordable (the "Castle
  // Ready" nudge) - nothing once truly in open free-roam with other moves still available, and
  // nothing while this.tutorialWaiting is true (the step's move isn't affordable yet - that's
  // the End Turn shine's job to flag, not this tile's; glinting a tile the player can't act on
  // yet would send a contradictory "tap here" signal).
  updateTutorialTileTarget() {
    let target = null;
    if (!this.tutorialFree) {
      const step = this.tutorialSteps[this.tutorialStep];
      if (step && this.actionFor(step.r, step.c)) target = { r: step.r, c: step.c };
    } else {
      let aiTiles = 0;
      this.forEachTile((t) => { if (t.owner === 2) aiTiles += 1; });
      if (aiTiles === 1 && this.actionFor(this.homeAI.r, this.homeAI.c)) {
        target = { r: this.homeAI.r, c: this.homeAI.c };
      }
    }
    const cur = this.tutorialTileGlint;
    if (cur && target && cur.r === target.r && cur.c === target.c) return; // already glinting the right tile
    this.stopTutorialTileGlint();
    if (target) this.startTutorialTileGlint(target.r, target.c);
  }

  // Same 'glintBar' sweep + geometry-mask technique as startEndBtnShine, aimed at a board tile
  // instead of a button - works for any tile (resource, empty, or the enemy home) since the mask
  // is just the tile's own square, not the icon's silhouette.
  startTutorialTileGlint(r, c) {
    const x = this.originX + c * TILE + TILE / 2;
    const y = this.originY + r * TILE + TILE / 2;
    const size = TILE - 4;
    const maskGfx = this.make.graphics({}, false);
    maskGfx.fillStyle(0xffffff, 1).fillRect(x - size / 2, y - size / 2, size, size);
    const mask = maskGfx.createGeometryMask();
    const barW = size * 0.45;
    const shine = this.add.image(x - size / 2 - barW, y, 'glintBar')
      .setDisplaySize(barW, size * 1.7).setRotation(Phaser.Math.DegToRad(24))
      .setMask(mask).setBlendMode(Phaser.BlendModes.ADD).setAlpha(0.147).setDepth(3.5);
    const tween = this.tweens.add({
      targets: shine, x: x + size / 2 + barW, duration: 1875, delay: 1000, repeat: -1, repeatDelay: 700, ease: 'Sine.easeInOut',
    });
    this.tutorialTileGlint = { r, c, shine, tween, maskGfx };
  }

  stopTutorialTileGlint() {
    if (!this.tutorialTileGlint) return;
    this.tutorialTileGlint.tween.stop();
    this.tutorialTileGlint.shine.destroy();
    this.tutorialTileGlint.maskGfx.destroy();
    this.tutorialTileGlint = null;
  }

  // A diagonal light streak sweeping across the End Turn button, masked to its bounds - draws
  // the eye to it during the tutorial's "Waiting" nudge. Idempotent (safe to call every refresh).
  // Uses the same 'glintBar' texture as the resource-node glint sweep (see makeGlintTexture) -
  // a soft alpha-ramped bar (0->1->0 across its width) instead of a hard-edged rectangle, so it
  // fades in/out as it crosses rather than popping in and cutting off at the button's edges.
  startEndBtnShine() {
    if (this.endBtnShine) return;
    const w = this.endBtn.width, h = this.endBtn.height, x = this.endBtn.x, y = this.endBtn.y;
    const maskGfx = this.make.graphics({}, false);
    maskGfx.fillStyle(0xffffff, 1).fillRect(x - w / 2, y - h / 2, w, h);
    const mask = maskGfx.createGeometryMask();
    const barW = w * 0.28;
    const shine = this.add.image(x - w / 2 - barW, y, 'glintBar')
      .setDisplaySize(barW, h * 1.4).setRotation(Phaser.Math.DegToRad(24))
      .setMask(mask).setBlendMode(Phaser.BlendModes.ADD).setAlpha(0.15).setDepth(5);
    const tween = this.tweens.add({
      targets: shine, x: x + w / 2 + barW, duration: 1440, repeat: -1, repeatDelay: 1000, ease: 'Sine.easeInOut',
    });
    this.endBtnShine = { shine, tween, maskGfx };
  }

  stopEndBtnShine() {
    if (!this.endBtnShine) return;
    this.endBtnShine.tween.stop();
    this.endBtnShine.shine.destroy();
    this.endBtnShine.maskGfx.destroy();
    this.endBtnShine = null;
  }

  // The message panel itself: full width (portrait) / board width (wide), between the board
  // and the legend - see computeLayout() for the isTutorial geometry (msgBoxX/Top/W/H).
  buildTutorialBox() {
    this.panel(this.msgBoxX, this.msgBoxTop, this.msgBoxW, this.msgBoxH, 0x22263a, 10);
    this.refreshTutorialMessage();
  }

  // Renders a tutorial message string into the message box, word-wrapping it manually so a
  // <gold>/<wood>/<stone>/<special> token can render as one unbroken unit: a small tinted icon
  // immediately left of the bold resource name, with normal word-spacing on both sides (the
  // token is just treated as one more "word" by the wrap loop below). Rebuilds the box's
  // contents from scratch each call - cheap at this text length, and keeps the caller (just
  // pass the string you want shown) simple.
  renderTutorialMessage(text) {
    if (this.tutorialMsgCont) this.tutorialMsgCont.destroy();
    const pad = 20;
    const fs = this.isWide ? 19 : 22;
    const cont = this.add.container(this.msgBoxX + pad, this.msgBoxTop + pad);
    this.tutorialMsgCont = cont;
    this.layoutTutorialText(text, cont, this.msgBoxW - pad * 2, fs);
  }

  // Tokenizes <res>/<action> markup + **bold** spans (see assets/tutorial-messages.md's header
  // for the author-facing rules) and lays the result out as word-wrapped rich text inside `cont`
  // (added at whatever position the caller already gave it - this only works in local space).
  // Shared by the in-board tutorial message box (renderTutorialMessage) and the tutorial intro
  // popup (showTutorialIntro) so both get the same icons/bold/action-color rendering. Returns the
  // total height used, so a caller sizing a card around the text knows how tall to make it.
  layoutTutorialText(text, cont, maxW, fs) {
    const font = 'system-ui, -apple-system, sans-serif';
    const lineH = fs + 10;
    const SPACE = 6;
    const ICON_SPACE = SPACE - 2;   // tighter gap on the word-side of an icon token (left/right)
    const CASTLE_ICON_SPACE = ICON_SPACE - 6; // gap before a castle icon, tighter than other icons
    const ICON_LABEL_GAP = 2;       // gap between the icon glyph and its own bold label (was 4)
    const CASTLE_ICON_LABEL_GAP = -2; // castle icon overlaps slightly into its own "CASTLE" label
    const RES_LABEL = {
      gold: 'GOLD', wood: 'WOOD', stone: 'STONE', special: 'SPECIAL',
      'enemy-castle': 'CASTLE', 'player-castle': 'CASTLE',
    };
    // Castle tokens reuse the home-base icon (ic_home), tinted to match the same red/blue used
    // for owner tiles and flags elsewhere (see drawFlags()) instead of a per-resource tint.
    const RES_TEX = { 'enemy-castle': 'home', 'player-castle': 'home' };
    const RES_TINT = { 'enemy-castle': 0xe05668, 'player-castle': 0x4f7df0 };
    // Action tokens - no icon, just the word itself bold + colored to match its board glow
    // (ACTION_COLOR) and the How-to-Play legend, so "Expand"/"Build"/"Upgrade"/"Siege" read as
    // a labeled term wherever they appear in tutorial copy.
    const ACTION_LABEL = { expand: 'expand', build: 'build', upgrade: 'upgrade', siege: 'siege' };
    const ACTION_LABEL_COLOR = { expand: '#ffce3a', build: '#57c97a', upgrade: '#b06bff', siege: '#ff6a3d' };

    // Tokenize into plain words and <res>/<action> units, in order. **bold** spans are stripped
    // of their asterisks first and flag the plain words inside them as bold. `glue` marks a unit
    // that had no whitespace before it in the source (e.g. punctuation right after a tag, "<build>,")
    // so the layout below can skip the normal word-gap and render it flush, same as plain text.
    const units = [];
    let pendingGlue = false; // true when the previous literal chunk ended with no trailing space
    text.split(/(\*\*[^*]+\*\*)/).filter(Boolean).forEach((seg) => {
      const bold = /^\*\*[^*]+\*\*$/.test(seg);
      const inner = bold ? seg.slice(2, -2) : seg;
      let firstInSeg = true; // the ** strip erases the real gap before/after a bold span, so its
      // first sub-part can't check its own leading whitespace (there isn't any - "**" sat flush
      // against the word) - it must trust pendingGlue, which already captured the true gap from
      // the segment before this one.
      inner.split(/(<gold>|<wood>|<stone>|<special>|<enemy-castle>|<player-castle>|<expand>|<build>|<upgrade>|<siege>)/).forEach((part) => {
        if (part === '') return;
        const m = part.match(/^<(gold|wood|stone|special|enemy-castle|player-castle)>$/);
        const a = part.match(/^<(expand|build|upgrade|siege)>$/);
        if (m) { units.push({ res: m[1], glue: pendingGlue }); pendingGlue = false; firstInSeg = false; }
        else if (a) { units.push({ action: a[1], glue: pendingGlue }); pendingGlue = false; firstInSeg = false; }
        else {
          const localLeadingGlue = !/^\s/.test(part);
          const leadingGlue = (bold && firstInSeg) ? pendingGlue : localLeadingGlue;
          const trailingGlue = !/\s$/.test(part);
          part.split(/\s+/).filter(Boolean).forEach((word, i) => {
            units.push({ word, bold, glue: i === 0 ? leadingGlue : false });
          });
          pendingGlue = trailingGlue;
          firstInSeg = false;
        }
      });
    });

    const measure = this.add.text(0, 0, '', { fontFamily: font, fontSize: `${fs}px` }).setVisible(false);
    let cx = 0, cy = 0, prevWasIcon = false;
    units.forEach((u) => {
      let w, icon = null, label = null;
      const isCastle = u.res === 'enemy-castle' || u.res === 'player-castle';
      if (u.res) {
        // Castle icons read small at the normal resource-icon size (fine crenellation detail),
        // so they get a larger display size - text baseline stays put, icon just grows around it.
        const ISIZE = isCastle ? fs + 14 : fs + 4;
        icon = this.add.image(0, 0, `ic_${RES_TEX[u.res] || u.res}`).setDisplaySize(ISIZE, ISIZE)
          .setTint(RES_TINT[u.res] || TINT[u.res]);
        label = this.add.text(0, 0, RES_LABEL[u.res], {
          fontFamily: font, fontSize: `${fs}px`, fontStyle: 'bold', color: '#ffffff',
        });
        const labelGap = isCastle ? CASTLE_ICON_LABEL_GAP : ICON_LABEL_GAP;
        w = ISIZE + labelGap + label.width;
      } else if (u.action) {
        measure.setFontStyle('bold');
        measure.setText(ACTION_LABEL[u.action]);
        w = measure.width;
      } else {
        measure.setFontStyle(u.bold ? 'bold' : 'normal');
        measure.setText(u.word);
        w = measure.width;
      }
      // Tighter gap on either side of an icon token (left of the icon, right of its label);
      // a castle icon's own left-side gap is tighter still (right side is a normal word gap).
      let gap = (u.res || prevWasIcon) ? ICON_SPACE : SPACE;
      if (isCastle) gap = CASTLE_ICON_SPACE;
      if (u.glue) gap = 0; // no whitespace in the source here (e.g. punctuation glued to a tag)
      if (cx > 0 && cx + gap + w > maxW) { cx = 0; cy += lineH; }
      else if (cx > 0) cx += gap;

      if (u.res) {
        const labelGap = isCastle ? CASTLE_ICON_LABEL_GAP : ICON_LABEL_GAP;
        icon.setPosition(cx + icon.displayWidth / 2, cy + lineH / 2);
        label.setPosition(cx + icon.displayWidth + labelGap, cy + lineH / 2).setOrigin(0, 0.5);
        cont.add([icon, label]);
      } else if (u.action) {
        cont.add(this.add.text(cx, cy + lineH / 2, ACTION_LABEL[u.action], {
          fontFamily: font, fontSize: `${fs}px`, fontStyle: 'bold', color: ACTION_LABEL_COLOR[u.action],
        }).setOrigin(0, 0.5));
      } else {
        cont.add(this.add.text(cx, cy + lineH / 2, u.word, {
          fontFamily: font, fontSize: `${fs}px`, fontStyle: u.bold ? 'bold' : 'normal', color: '#e7e9f0',
        }).setOrigin(0, 0.5));
      }
      cx += w;
      prevWasIcon = !!u.res;
  ...
  // One-time welcome popup shown when the guided tutorial starts, styled like the game's other
  // modal cards (dim scrim, rounded panel, close x top-right, primary button bottom) - its
  // message comes from the "Intro" section of assets/tutorial-messages.md (see that file's
  // header), rendered with the same icon/bold/action-color rich text as the in-board message box.
  showTutorialIntro() {
    const font = 'system-ui, -apple-system, sans-serif';
    ...
    const textW = cardW - 80;

    // Pass 1: lay the rich text out into an offscreen container just to measure how tall it
    // renders at this width/font size, so the card can be sized to fit it exactly.
    const probe = this.add.container(0, 0).setVisible(false);
    const textH = this.layoutTutorialText(this.tutorialIntroMsg, probe, textW, fs);
    probe.destroy();

    const TITLE_H = 90, BTN_AREA_H = 110;
    const cardH = TITLE_H + textH + BTN_AREA_H;
    const cardX = CX - cardW / 2, cardY = Math.max(36, Math.round((H - cardH) / 2));

    const cont = this.add.container(0, 0).setDepth(60);
    const dim = this.add.rectangle(CX, H / 2, W, H, 0x000000, 0.74).setInteractive();
    cont.add(dim);
    cont.add(this.panel(cardX, cardY, cardW, cardH, 0x22263a, 20));
    // Swallow taps on the card itself so they don't fall through to the dim scrim behind it.
    cont.add(this.add.rectangle(CX, cardY + cardH / 2, cardW, cardH, 0x000000, 0.001).setInteractive());

    cont.add(this.add.text(CX, cardY + 46, 'Tutorial', {
      fontFamily: HEAD, fontSize: '40px', fontStyle: 'bold', color: '#e7e9f0',
    }).setOrigin(0.5));

    const textCont = this.add.container(cardX + 40, cardY + TITLE_H);
    this.layoutTutorialText(this.tutorialIntroMsg, textCont, textW, fs);
    cont.add(textCont);

    const hide = () => cont.destroy(true);

    // Close × (top-right).
    const cx0 = cardX + cardW - 40, cy0 = cardY + 40;
    const closeBase = 0x39405c, closeHover = lighten(closeBase);
    const closeCircle = this.add.circle(cx0, cy0, 24, closeBase).setInteractive({ useHandCursor: true });
    const closeX = this.add.text(cx0, cy0 - 1, '✕', { fontFamily: font, fontSize: '26px', color: '#e7e9f0' }).setOrigin(0.5);
    closeCircle.on('pointerover', () => { closeCircle.setFillStyle(closeHover); closeX.setScale(1.1); });
    closeCircle.on('pointerout', () => { closeCircle.setFillStyle(closeBase); closeX.setScale(1); });
    closeCircle.on('pointerdown', hide);
    cont.add([closeCircle, closeX]);

    // "Got it" button, bottom of the card - a rounded pill matching the other popups' buttons
    // (ui.js's makeButton draws the same fillRoundedRect(radius = h/2) shape).
    const btnW = 240, btnH = 64, btnCx = CX, btnCy = cardY + cardH - 56;
    const btnGx = btnCx - btnW / 2, btnGy = btnCy - btnH / 2, btnRadius = btnH / 2;
    const btnColor = 0x3d6cff, btnHover = lighten(btnColor);
    const btnG = this.add.graphics().fillStyle(btnColor, 1).fillRoundedRect(btnGx, btnGy, btnW, btnH, btnRadius);
    const btnHit = this.add.rectangle(btnCx, btnCy, btnW, btnH, 0x000000, 0.001).setInteractive({ useHandCursor: true });
    const btnText = this.add.text(btnCx, btnCy, 'Got it', {
      fontFamily: HEAD, fontSize: '26px', fontStyle: 'bold', color: '#ffffff',
    }).setOrigin(0.5);
    btnHit.on('pointerover', () => { btnG.clear().fillStyle(btnHover, 1).fillRoundedRect(btnGx, btnGy, btnW, btnH, btnRadius); btnText.setScale(1.04); });
    btnHit.on('pointerout', () => { btnG.clear().fillStyle(btnColor, 1).fillRoundedRect(btnGx, btnGy, btnW, btnH, btnRadius); btnText.setScale(1); });
    btnHit.on('pointerdown', hide);
    cont.add([btnG, btnHit, btnText]);
  }
  ...
  // The corner-wrap + independently-picked bumps above can occasionally stack ocean tiles on
  // every side of a land/shoal tile at once, boxing it in - a resource placed there afterward
  // (or already sitting there from a redraw) would be permanently unreachable, since claiming
  // requires 4-directional adjacency to owned territory and ocean tiles are never claimable.
  // Flood-fills outward from both homes across non-ocean tiles; anything the fill never
  // reaches is sealed into deep water before nodes are ever placed, so the dead spot simply
  // never gets a resource instead of trapping one.
  sealUnreachablePockets() {
    const seen = Array.from({ length: GRID_H }, () => new Array(GRID_W).fill(false));
    const queue = [this.homePlayer, this.homeAI];
    ...
    for (let head = 0; head < queue.length; head++) {
      ...
      for (const [dr, dc] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
        const nr = r + dr, nc = c + dc;
        if (nr < 0 || nr >= GRID_H || nc < 0 || nc >= GRID_W) continue;
        if (seen[nr][nc] || this.tiles[nr][nc].ocean) continue;
        seen[nr][nc] = true;
    ...
    for (let r = 0; r < GRID_H; r++) {
      for (let c = 0; c < GRID_W; c++) {
        if (!seen[r][c] && !this.tiles[r][c].ocean && !this.tiles[r][c].home) pocketCells.push({ r, c });
      }
    }
    for (const { r, c } of pocketCells) this.tiles[r][c].ocean = true;

    // A sealed pocket is, by construction, walled off by original ocean and/or the board edge,
    // so at least one cell in it always borders a tile that already has a region - propagate it
    // inward (iterating since a pocket can be more than one tile deep).
    let progress = true;
    while (progress) {
      progress = false;
  ...
  buildOceanWater() {
    ...
    // ONE shared clip mask per water type instead of one GeometryMask per tile. Every masked
    // GameObject forces its own stencil-buffer draw pass in WebGL and can't batch with anything
    // else - with ~20+ ocean/shoal tiles at 2 masked layers each, that was ~40-50 extra masked
    // draws every single frame, which is what was cratering FPS on the shoreline level (the
    // river level has no per-tile masks and stays smooth). Instead: two invisible Graphics
    // objects, each holding the union of every relevant tile's current rounded-corner shape,
    // and one Container per water type whose children (the actual swell/ripple/foam sprites)
    // all get clipped together by that single shared mask - 2 stencil passes total, not 40+.
    // A GeometryMask re-renders its source Graphics live each frame, so redrawing the same
    // Graphics in place (updateOceanSwellMasks) updates every tile's clip at once with no
    // per-tile mask objects to create or tear down.
    this.oceanMaskGfx = this.add.graphics().setVisible(false);
    this.shoalMaskGfx = this.add.graphics().setVisible(false);
    this.oceanSwellContainer = this.add.container(0, 0).setDepth(2)
      .setMask(this.oceanMaskGfx.createGeometryMask());
    // Ripple then foam share one container/mask - within a tile ripple is always added before
    // foam (see setupShoalTileFX), so foam still renders on top of its own tile's ripple; order
    // between different tiles doesn't matter since water tiles never overlap on screen.
    this.shoalContainer = this.add.container(0, 0).setDepth(2)
      .setMask(this.shoalMaskGfx.createGeometryMask());
  ...
  setupOceanTileFX(r, c) {
    ...
    const swell = (key, fromX, toX, dur) => {
      // Clipped by the shared oceanSwellContainer mask (see buildOceanWater), not a per-tile
      // mask - ocean tile rounding is tide-dependent (computeCornerPlan), and that shared mask
      // is what updateOceanSwellMasks() redraws on every tide-phase change.
      const sp = this.add.tileSprite(v.rect.x, v.rect.y, TILE - 4, TILE - 4, key);
      this.oceanSwellContainer.add(sp);
  ...
  setupShoalTileFX(r, c) {
    ...
    const rip = this.add.tileSprite(v.rect.x, v.rect.y, TILE - 4, TILE - 4, 'shoalRipple')
      .setAlpha(0.5).setVisible(false);
    this.shoalContainer.add(rip); // clipped by the shared shoalContainer mask, see buildOceanWater
    ...
    v.foam = this.add.image(v.rect.x, v.rect.y, 'foamStripes')
      .setDisplaySize(TILE - 4, TILE - 4).setVisible(false);
    this.shoalContainer.add(v.foam); // added after ripple so it renders on top, see above
  ...
  teardownTileFX(r, c) {
    const v = this.views[r][c];
    // Sprites live in the shared oceanSwellContainer/shoalContainer now (see buildOceanWater) -
    // no per-tile mask to destroy, just remove-and-destroy the child itself.
    if (v.swellSprites) {
      v.swellSprites.forEach((sp) => this.oceanSwellContainer.remove(sp, true));
      v.swellSprites = null;
    }
    if (v.ripple) {
      this.shoalContainer.remove(v.ripple, true);
      v.ripple = null;
    }
    if (v.foam) {
      this.shoalContainer.remove(v.foam, true);
      v.foam = null;
    }
    v.glitterDots = null;
  }
  ...
  // The sweep-highlight image masked to a node icon's own silhouette (see buildBoard()'s comment
  // on Glint Sweep). Pulled out so tileEditor.js can lazily create one too when dev-mode paints a
  // resource onto a tile that started plain - buildBoard() only makes these for tiles that already
  // had a resource at board-generation time, so a tile painted later would otherwise have no
  // v.glint for drawNodeGlints() to touch.
  createNodeGlint(x, y, k, icon) {
    const glint = this.add.image(x, y, 'glintBar')
      .setDisplaySize(14 * k, 44 * k).setRotation(Phaser.Math.DegToRad(22))
      .setBlendMode(Phaser.BlendModes.ADD).setAlpha(0.10).setDepth(4.3);
    glint.setMask(icon.createBitmapMask());
    glint.baseX = x;
    return glint;
  }
  ...
  buildBoard() {
    ...
    for (let r = 0; r < GRID_H; r++) {
      ...
      for (let c = 0; c < GRID_W; c++) {
        ...
        // Resource nodes only (not home - the flag covers that): a soft highlight bar that
        // sweeps across the icon on a slow cycle ("Glint Sweep", stage/home-node-animations.html
        // variant C). Masked to the icon's OWN silhouette via a BitmapMask (icon as the mask
        // source), not just a bounding circle, so the shine only ever shows over the actual
        // glyph - and since BitmapMask samples the icon live every frame, it stays correct even
        // though refresh() swaps the icon's texture per resource type after buildBoard() runs.
        // Phase-offset per tile below (drawNodeGlints) so nodes don't flash in sync.
        const glint = this.tiles[r][c].resource ? this.createNodeGlint(x, y, k, icon) : null;

        row.push({ rect, icon, badge, income, glint, hoverGlow });
      }
      this.views.push(row);
    }
    // One Graphics layer for both home tiles' waving flags, redrawn every frame in update().
    // Depth 4.5 sits above the watchtower icon (depth 4) but below badges/income text (5-6).
    this.flagGfx = this.add.graphics().setDepth(4.5);
  ...
  // fixes, not a case this rule needs to special-case away. This is the PERMANENT classification:
  // it governs land tiles' concave corners and a shoal tile's OWN convex corners (both fixed for
  // the whole game). Ocean tiles' own convex corners use oceanSeesWater() instead (tide-aware, so
  // an ocean tile can round toward a dry shoal and flatten when it floods) - see that function's
  // comment for why the two can coexist without the old double-circle bug (an earlier version had
  // land tiles use this permanent rule while ocean tiles used a live tide-dependent one that
  // could each independently carve a rounded corner at the same grid vertex - two overlapping
  // circles instead of one clean curve, showing up as a small stray colored dot at the shoreline).
  isShoreWater(r, c) {
    const t = this.tiles[r] && this.tiles[r][c];
    return !!(t && (t.ocean || t.shoal));
  }

  // Ocean tiles' OWN corner decision (see computeCornerPlan) uses this instead of isShoreWater:
  // a neighboring shoal only reads as "water" to an ocean tile while that shoal is actually
  // submerged - dry shoal reads as land-like, so the ocean tile rounds a corner toward it (and
  // flattens the instant the tide floods that shoal), matching the pre-v0.19 ocean behavior.
  // Safe to make tide-dependent again WITHOUT reintroducing the old double-circle bug: a shoal
  // tile's own corner decision (isShoreWater, used by both land's concave rounding and shoal's
  // own convex rounding) always treats its ocean neighbor as permanent water, so the shoal side
  // of an ocean/shoal boundary never independently tries to round there - only the ocean side
  // ever draws a corner at that shared vertex, so the two can no longer disagree.
  oceanSeesWater(r, c) {
    const t = this.tiles[r] && this.tiles[r][c];
    if (!t) return false;
    if (t.ocean) return true;
    if (t.shoal) return this.isSubmerged(t);
    return false;
  }
  ...
  // dry shoal. Land and shoal corners are fixed for the whole game (isShoreWater is permanent,
  // terrain-based); ocean corners are the one tide-dependent piece (oceanSeesWater above), so
  // this must be recomputed on every tide-phase change, not just when terrain changes - see the
  // computeCornerPlan() calls in advanceTide().
  computeCornerPlan() {
    ...
    this.forEachTile((t, r, c) => {
      ...
      // Ocean tiles judge their own neighbors tide-aware (oceanSeesWater); every other tile
      // (land's concave check, a shoal tile's own convex check) keeps the permanent rule.
      const seesWater = t.ocean ? (rr, cc) => this.oceanSeesWater(rr, cc) : (rr, cc) => this.isShoreWater(rr, cc);
      const up = seesWater(r - 1, c), down = seesWater(r + 1, c);
      const left = seesWater(r, c - 1), right = seesWater(r, c + 1);
      const flags = water
  ...
  // Redraws the two shared water clip masks (see buildOceanWater) since computeCornerPlan just
  // changed which corners are rounded. A GeometryMask re-renders its source Graphics live each
  // frame, so clearing and redrawing oceanMaskGfx/shoalMaskGfx in place updates every ocean/
  // shoal tile's clip at once - no per-tile mask objects to create or destroy. Shoal corners are
  // actually fixed for the whole game (only ocean tiles round tide-dependently, see
  // oceanSeesWater), so redrawing shoalMaskGfx here is technically redundant work, but it's two
  // rounds a game and cheap - simpler than a separate rebuild path just for the ocean half.
  updateOceanSwellMasks() {
    const half = (TILE - 4) / 2;
    const drawInto = (g, wants) => {
      g.clear();
      this.forEachTile((t, r, c) => {
        if (!wants(t)) return;
        const v = this.views[r][c];
        const plan = (this.cornerPlan[r] && this.cornerPlan[r][c]) || { tl: 0, tr: 0, bl: 0, br: 0 };
        g.fillStyle(0xffffff, 1).fillRoundedRect(v.rect.x - half, v.rect.y - half, half * 2, half * 2, plan);
      });
    };
    if (this.oceanMaskGfx) drawInto(this.oceanMaskGfx, (t) => t.ocean);
    if (this.shoalMaskGfx) drawInto(this.shoalMaskGfx, (t) => t.shoal);
  }
  ...
  // Sweeps each resource node's glint bar left-to-right, dwelling off-screen (clipped away by
  // its mask) for GAP_MS between each SWEEP_MS-long pass - split into two named constants
  // instead of one cycle+fraction so "how often" and "how fast" can be tuned independently.
  // Alpha ramps with the bar's own position rather than staying flat: faint (0.05) as it enters/
  // leaves near the icon's edge, peaking at 0.15 as it crosses the center - much subtler overall
  // than the first pass (was a flat 0.85) and reads as a light catching the icon, not a flash.
  drawNodeGlints() {
    const SWEEP_MS = 1300, GAP_MS = 7800, cycle = SWEEP_MS + GAP_MS;
    const k = this.tileK, barSpan = 44 * k;
    this.forEachTile((tile, r, c) => {
      const v = this.views[r][c];
      if (!v.glint) return; // no glint object yet (see createNodeGlint) - nothing to hide or animate
      if (!tile.resource) { v.glint.setVisible(false); return; } // node removed - hide the leftover sweep sprite
      if (!v.icon.visible) return;
      v.glint.setVisible(true);
      const phase = (r * GRID_W + c) * 400;
      const p = (this.time.now + phase) % cycle;
      const f = p < GAP_MS ? 0 : (p - GAP_MS) / SWEEP_MS;
      v.glint.x = v.glint.baseX + Phaser.Math.Linear(-barSpan, barSpan, f);
      const centerCloseness = 1 - Math.abs(f - 0.5) * 2; // 0 at either edge, 1 dead center
      v.glint.setAlpha(0.10 + centerCloseness * 0.15); // 0.10 at edges -> 0.25 at center
    });
  }

  // Small waving flag centered on each home tile, sticking up out of the watchtower's parapet -
  // "Ripple Wave" from stage/home-node-animations.html (variant A): the cloth's free edge sways
  // on a per-frame sine skew pinned at the pole, x running rightward while y ripples. Reads
  // tile.owner live rather than caching it, so the flag recolors instantly if a home is captured.
  drawFlags() {
    const g = this.flagGfx;
    g.clear();
    const k = this.tileK;
    // ic_home's watchtower silhouette (assets/icons/watchtower.svg) sits inside an 80-unit-tall
    // viewBox but the crenellations only reach up to y=-19 of it - so the parapet's visual top,
    // at the icon's displaySize (66 * k), is (19/80) * 66 * k below the icon's own center.
    const parapetOffset = (19 / 80) * 66 * k;
    const poleH = 18 * k, poleW = 2.2 * k, flagW = 15 * k, flagH = 10 * k, cols = 6;
    const t = this.time.now;
    for (const h of [this.homePlayer, this.homeAI]) {
      const tile = this.tiles[h.r][h.c];
      const v = this.views[h.r][h.c];
      const color = tile.owner === 2 ? 0xe05668 : 0x4f7df0;
      // Pole is centered on the icon (not offset to a side) and its base sits right at the
      // parapet; poleH + the ripple's own sway comfortably clears the tile edge above it.
      const poleX = v.rect.x, parapetY = v.rect.y + HOME_ICON_Y_NUDGE * k - parapetOffset, poleTop = parapetY - poleH;
      g.fillStyle(0x6b5636, 1).fillRect(poleX - poleW / 2, poleTop, poleW, poleH);
      const top = [], bot = [];
      for (let i = 0; i <= cols; i++) {
        const f = i / cols;
        const sway = Math.sin(t / 260 + f * 2.4) * flagH * 0.3 * f;
        top.push({ x: poleX + f * flagW, y: poleTop + 2 * k + sway });
        bot.push({ x: poleX + f * flagW, y: poleTop + flagH + 2 * k + sway });
      }
      g.fillStyle(color, 1).fillPoints(top.concat(bot.reverse()), true);
    }
  }
  ...
  buildGameOverOverlay() {
    ...
    // Tutorial ends back at the title screen (there's no level to replay); a normal game
    // offers Play Again, back to level select.
    const isTutorial = this.level === 'tutorial';
    const againBtn = this.add.rectangle(CX, textY + 160, 320, 96, 0x3d6cff).setInteractive({ useHandCursor: true });
    const againText = this.add.text(CX, textY + 160, isTutorial ? 'Close' : 'Play Again', {
      fontFamily: HEAD, fontSize: '38px', fontStyle: 'bold', color: '#ffffff',
    }).setOrigin(0.5);
    ...
    againBtn.on('pointerdown', () => {
      ...
      this.scene.start(isTutorial ? 'TitleScene' : 'LevelSelectScene');
    });
  ...
  buildNewGameConfirm() {
    ...
    // Tutorial has no level to reselect - leaving it always goes back to the title, matching
    // the game-over "Close" button's exit route.
    confirmBtn.on('pointerdown', () => {
      this.sfx.play('newgame');
      this.scene.start(this.level === 'tutorial' ? 'TitleScene' : 'LevelSelectScene');
    });
  ...
  runAITurn() {
    this.current = 2;
    this.addIncome(2);
    const scripted = this.level === 'tutorial' ? this.tutorialAIClaim() : null;
    this.refresh();
    if (scripted) {
      // Same beat as a real AI move (see aiStep) - juice after the owner flip has rendered,
      // then a pause before continuing so it doesn't collide with the End Turn transition.
      this.juiceAction(scripted);
      this.time.delayedCall(280, () => this.aiStep(0));
      return;
    }
  ...
  // Two scripted AI land-grabs tied to the tutorial's own pacing (see TUTORIAL_STEPS), instead
  // of the AI owning (1,0)/(0,1) from turn one: it claims (0,1) once Step 2 is done, then (1,0)
  // once Step 4 is done - each check fires every AI turn but only acts once, guarded by
  // this.tutorialAIClaimed (NOT live tile ownership - once Step 7 has the player siege (0,1)
  // back off the AI, its owner is 1 again, and a live-ownership check would read that as "never
  // claimed" and have the AI grab it right back). Flips the tile directly (no resource cost -
  // this is scripted territory, not a real economy move) and returns a move object for
  // juiceAction to animate, or null if neither threshold has been newly reached.
  tutorialAIClaim() {
    if (this.tutorialStep >= 2 && !this.tutorialAIClaimed[0]) {
      this.tutorialAIClaimed[0] = true;
      this.tiles[0][1].owner = 2;
      return { type: 'claim', r: 0, c: 1 };
    }
    if (this.tutorialStep >= 4 && !this.tutorialAIClaimed[1]) {
      this.tutorialAIClaimed[1] = true;
      this.tiles[1][0].owner = 2;
      return { type: 'claim', r: 1, c: 0 };
    }
    return null;
  }
  ...
  computeIncome(p) {
    ...
    // Tutorial's enemy home never generates income (see setupState()'s zero starting stock) -
    // keeps it permanently unable to act, not just unarmed at kickoff.
    if (this.level === 'tutorial' && p === 2) return inc;
    this.forEachTile((t) => {
      if (t.owner !== p) return;
  ...
  onTileTap(r, c) {
    ...
    if (this.level === 'tutorial' && !this.tutorialAllows(r, c)) return;
    this.sfx.unlock();
    const t = this.tiles[r][c];
    ...
    this.hasActed = true;
    if (this.level === 'tutorial') this.advanceTutorial(move);
    this.refresh();
    this.juiceAction(move);
    this.checkWin();
  ...
  refresh() {
    ...
    this.forEachTile((t, r, c) => {
      ...
      if (t.resource) {
        ...
        v.icon.setTexture('ic_home').setDisplaySize(66 * this.tileK, 66 * this.tileK).setTint(0xffffff).setVisible(true)
          .setY(v.rect.y + HOME_ICON_Y_NUDGE * this.tileK);
      } else {
    ...
    // Tutorial message box: re-derived every render so it always matches current affordability
    // (see computeTutorialMessage()) - covers both AI turns and End Turn income landing.
    if (this.level === 'tutorial') this.refreshTutorialMessage();
  }
  ...
  actionFor(r, c) {
    ...
    if (this.level === 'tutorial' && !this.tutorialAllows(r, c)) return null;
    const t = this.tiles[r][c];
  ...
  showTooltip(r, c) {
    ...
    const parts = [];
    if (gain.icon) {
      // The final home-siege tooltip ("WIN - take their base") gets a bigger castle icon than a
      // normal resource gain icon - it's the whole game riding on that tap, worth the extra weight.
      const iconSize = gain.icon === 'home' ? 44 : 32;
      const im = this.add.image(0, 0, `ic_${gain.icon}`).setDisplaySize(iconSize, iconSize).setTint(gain.tint).setOrigin(0, 0.5);
      parts.push({ o: im, w: iconSize });
    }
