...
// Display serif (Grenze) for the medieval feel - used ONLY on modal headings and buttons here.
// Everything else (rows, descriptions, section labels, about lines) stays FONT so it reads
// cleanly under the CRT filter. Georgia/serif fall back if the webfont hasn't loaded yet.
...
export function createTutorialOverlay(scene) {
  ...
  const show = () => {
    ...
    // Portrait (mobile) renders the whole canvas at ~0.54x, which made this card's text far too
    // small. Fix: build the card NARROWER in portrait, then uniformly scale the whole container UP
    // to nearly fill the screen - every element grows together (~1.25x) with no overflow or reflow.
    // Wide/desktop keeps the shrink-to-fit (0.8). The real scale `s` is computed once cardH is known.
    const wide = W > H;
    cont = scene.add.container(0, 0).setDepth(60);
    // Oversize the dim by the eventual scale so it still covers the full screen once the container
    // is scaled about the canvas center (a rough 1/s guess is fine - it only needs to over-cover).
    const dimK = wide ? 0.8 : 1.3;
    const dim = scene.add.rectangle(CX, H / 2, W / dimK, H / dimK, 0x000000, 0.74).setInteractive();
    dim.on('pointerdown', hide); // tap outside the card to dismiss
    cont.add(dim);

    const cardW = wide ? 620 : 560, cardX = CX - cardW / 2;
    ...
    const resRows = [
      ...
      { res: 'special', name: 'Special', desc: 'Provides all resource types' },
    ];
    const actRows = [
      { color: ACTION_COLOR.claim, name: 'Expand', desc: 'Take an empty neutral tile next to you.', res: 'gold', cost: 5 },
      { color: ACTION_COLOR.build, name: 'Build', desc: 'Take a neutral resource node, earning income.', res: 'wood', cost: 5 },
      { color: ACTION_COLOR.upgrade, name: 'Upgrade', desc: "Double a resource node's income.", res: 'stone', cost: 5 },
      { color: ACTION_COLOR.siege, name: 'Siege', desc: 'Capture an enemy tile. Nodes cost wood.', res: 'gold', cost: 10 },
    ];
    ...
    const RES_H = 60, ACT_H = 84, HDR_GAP = 20, PRE_HDR = 46, HEAD_H = 186;
    const cardH = HEAD_H + HDR_GAP + resRows.length * RES_H + PRE_HDR + HDR_GAP + actRows.length * ACT_H + 96;
    // Center the card in the canvas. On wide/desktop the card is taller than the 900px canvas, so we
    // let cardY go slightly negative and rely on the shrink-to-fit scale below (which scales about the
    // canvas center) to bring it fully on-screen - that keeps it truly vertically centered. Only in
    // portrait (s === 1, no shrink) do we floor the top at 36 so a tall card can't clip off the top.
    const cardY = wide ? Math.round((H - cardH) / 2) : Math.max(36, Math.round((H - cardH) / 2));
    ...
    cont.add(scene.add.text(CX, cardY + 46, 'How to Play', { fontFamily: HEAD, fontSize: '40px', fontStyle: 'bold', color: INK }).setOrigin(0.5));
    ...
    resRows.forEach((r) => {
      ...
      const g = roundRect(scene, rx, y + 8, cardW - 52, RES_H - 16, 0x1a1d2c, 10);
      const ic = scene.add.image(rx + 34, ry, `ic_${r.res}`).setDisplaySize(38, 38).setTint(TINT[r.res]).setOrigin(0.5);
      const name = scene.add.text(rx + 66, ry, r.name, { fontFamily: FONT, fontSize: '22px', fontStyle: 'bold', color: INK }).setOrigin(0, 0.5);
      const desc = scene.add.text(rx + 190, ry, r.desc, { fontFamily: FONT, fontSize: '17px', color: SUBTLE }).setOrigin(0, 0.5);
    ...
    // Two buttons: a guided Tutorial (fixed 4x4 board) alongside the plain dismiss.
    const btnGap = 16, btnW = (cardW - 52 - btnGap) / 2;
    const btnLx = cardX + 26 + btnW / 2, btnRx = cardX + cardW - 26 - btnW / 2;
    makeButton(scene, cont, btnLx, y + 48, btnW, 68, 'Play Tutorial', 0x39405c, INK, () => {
      hide();
      scene.scene.start('GameScene', { level: 'tutorial' });
    });
    makeButton(scene, cont, btnRx, y + 48, btnW, 68, 'Got it', ACCENT, '#ffffff', hide);

    // Now that the card's true height is known, size it to the canvas: shrink-to-fit on
    // wide/desktop (0.8), scale UP to nearly fill the screen on portrait (bounded by both width
    // and height so it never clips). Scaling about the canvas center keeps the card centered and
    // lets the oversized dim cover the screen.
    const s = wide ? 0.8 : Math.min((W * 0.97) / cardW, (H - 40) / cardH);
...
function startDlssSparkles(scene) {
  // Guard on `.scene`, not just truthiness: `scene.restart()` (New Game) destroys every
  // GameObject via Phaser's own scene shutdown, which nulls out `.scene` on the emitter but
  // leaves this stale reference sitting on the (reused) scene instance. A plain truthy check
  // would see that stale ref and skip recreating the emitter, silently leaving DLSS dark even
  // though the setting itself is still ON.
  if (scene._dlssParticles && scene._dlssParticles.scene) return;
...
function startGodrays(scene) {
  // Same stale-reference guard as startDlssSparkles - scene.restart() destroys the old beams
  // but leaves scene._godrays pointing at them.
  if (scene._godrays && scene._godrays.length && scene._godrays[0].scene) return;
...
// ------------------------------------------------------------------ Feedback
// A tiny bug/suggestion report form, reachable from Settings ▸ Feedback (see createSettingsOverlay
// below). TYPE is a pair of canvas toggle buttons (Bug/Suggestion); MESSAGE uses a real HTML
// <textarea> via Phaser's DOM element container (main.js enables `dom: { createContainer: true }`)
// since Phaser has no native multiline text input. Submits to the Cloudflare Worker in feedback.js,
// which turns it into a Trello card for triage.
export function createFeedbackOverlay(scene) {
  let cont = null;
  const hide = () => { if (cont) { cont.destroy(true); cont = null; } };
  ...
  const show = () => {
    ...
    // Label-to-field gap is shared by TYPE/select and MESSAGE/textarea so both rows read with
    // the same rhythm; textarea grows taller on mobile where thumb typing needs more room, so
    // everything below it (status line, buttons, card height) is derived rather than hardcoded.
    const LABEL_GAP = 16;
    const textareaH = port ? 260 : 220;
    const cardW = 560, cardX = CX - cardW / 2;
    const textareaTop = 250 + LABEL_GAP, textareaBottom = textareaTop + textareaH;
    const statusY = textareaBottom + 30, btnRowY = statusY + 70;
    const cardH = btnRowY + 32 + 32; // +32 button half-height, +32 bottom padding below it
    const cardY = Math.max(36, Math.round((H - cardH) / 2));
    cont.add(roundRect(scene, cardX, cardY, cardW, cardH, CARD, 20));
    const block = scene.add.rectangle(CX, cardY + cardH / 2, cardW, cardH, 0x000000, 0.001).setInteractive();
    cont.add(block);

    cont.add(scene.add.text(CX, cardY + 46, 'Feedback', { fontFamily: HEAD, fontSize: '40px', fontStyle: 'bold', color: INK }).setOrigin(0.5));
    makeClose(scene, cont, cardX + cardW - 40, cardY + 40, hide);
    cont.add(scene.add.text(CX, cardY + 92, 'Found a bug or have an idea? Let us know below.',
      { fontFamily: FONT, fontSize: port ? '19px' : '17px', color: SUBTLE, align: 'center', wordWrap: { width: cardW - 80 } }).setOrigin(0.5));

    const lx = cardX + 40, fieldW = cardW - 80;
    const section = (y, s) =>
      cont.add(scene.add.text(lx, y, s, { fontFamily: FONT, fontSize: port ? '18px' : '15px', fontStyle: 'bold', color: '#7c8398' }).setOrigin(0, 0.5));

    // --- TYPE toggle (Bug / Suggestion) ---
    section(cardY + 156, 'TYPE');
    let feedbackType = 'Bug';
    const typeGap = 16, typeBtnW = (fieldW - typeGap) / 2, typeY = cardY + 196;
    const typeBtns = {};
    const redrawType = () => {
      for (const [v, b] of Object.entries(typeBtns)) {
        const active = v === feedbackType;
        b.g.clear().fillStyle(active ? ACCENT : 0x39405c, 1).fillRoundedRect(b.gx, b.gy, typeBtnW, 48, 24);
        b.t.setColor(active ? '#ffffff' : SUBTLE);
      }
    };
    const makeTypeBtn = (cx, label, value) => {
      const gx = cx - typeBtnW / 2, gy = typeY - 24;
      const g = roundRect(scene, gx, gy, typeBtnW, 48, 0x39405c, 24);
      const hit = scene.add.rectangle(cx, typeY, typeBtnW, 48, 0x000000, 0.001).setInteractive({ useHandCursor: true });
      const t = scene.add.text(cx, typeY, label, { fontFamily: FONT, fontSize: port ? '18px' : '16px', fontStyle: 'bold', color: SUBTLE }).setOrigin(0.5);
      hit.on('pointerdown', () => { feedbackType = value; sfx.play('claim'); redrawType(); });
      cont.add([g, hit, t]);
      typeBtns[value] = { g, t, gx, gy };
    };
    makeTypeBtn(CX - typeGap / 2 - typeBtnW / 2, 'Bug', 'Bug');
    makeTypeBtn(CX + typeGap / 2 + typeBtnW / 2, 'Suggestion', 'Suggestion');
    redrawType();

    // --- MESSAGE textarea ---
    section(cardY + 250, 'MESSAGE');
    const textareaEl = document.createElement('textarea');
    textareaEl.className = 'fh-textarea';
    textareaEl.style.width = `${fieldW}px`;
    textareaEl.style.height = `${textareaH}px`;
    textareaEl.style.fontSize = port ? '19px' : '16px';
    textareaEl.maxLength = 2000;
    textareaEl.placeholder = "What happened, or what would you like to see?";
    const textareaDom = scene.add.dom(CX, cardY + textareaTop + textareaH / 2, textareaEl).setOrigin(0.5);
    cont.add(textareaDom);

    // Honeypot: a real (but off-screen, unreachable) field a scraping bot filling every input
    // on the page will populate; a real player never sees or fills it. Named `website`, a
    // common bait name for these. The Worker silently drops the submission if it's non-empty
    // instead of rejecting it outright, so bots get no signal they were caught.
    const hpEl = document.createElement('input');
    hpEl.type = 'text';
    hpEl.name = 'website';
    hpEl.tabIndex = -1;
    hpEl.autocomplete = 'off';
    hpEl.className = 'fh-hp';
    const hpDom = scene.add.dom(CX, cardY, hpEl);
    cont.add(hpDom);

    // Status line: validation errors / in-flight / success, between the form and the buttons.
    const status = scene.add.text(CX, cardY + statusY, '',
      { fontFamily: FONT, fontSize: '16px', color: SUBTLE, align: 'center', wordWrap: { width: fieldW } }).setOrigin(0.5);
    cont.add(status);

    // --- Cancel / Submit ---
    const btnY = cardY + btnRowY, gap = 16, btnW = (fieldW - gap) / 2;
    const cancelBtn = makeButton(scene, cont, CX - gap / 2 - btnW / 2, btnY, btnW, 64, 'Cancel', 0x39405c, INK, hide);
    let submitting = false;
    const submitBtn = makeButton(scene, cont, CX + gap / 2 + btnW / 2, btnY, btnW, 64, 'Submit', ACCENT, '#ffffff', async () => {
      if (submitting) return;
      const message = textareaEl.value.trim();
      if (!message) { status.setColor('#ff6a6a').setText('Please enter a message.'); return; }
      submitting = true;
      status.setColor(SUBTLE).setText('Sending...');
      const result = await submitFeedback({ type: feedbackType, message, hp: hpEl.value });
      submitting = false;
      // cont may have been destroyed (dismissed mid-request) - bail rather than touch dead objects.
      if (!cont) return;
      if (result.ok) {
        sfx.play('special');
        status.setColor('#7be08a').setText('Thanks! Feedback sent.');
        scene.time.delayedCall(900, hide);
      } else {
        status.setColor('#ff6a6a').setText(result.error);
      }
    });
    void cancelBtn; void submitBtn; // returned hit rects, unused here (onTap already wired)
  };
...
export function createSettingsOverlay(scene, { onHowToPlay } = {}) {
  let cont = null;
  const hide = () => { if (cont) { cont.destroy(true); cont = null; } };
  // Feedback is only ever reached from here, so it's owned by this overlay rather than
  // requiring every scene that calls createSettingsOverlay to also wire up its own.
  const feedback = createFeedbackOverlay(scene);
  ...
  const show = () => {
    ...
    // Portrait renders at ~0.54x, so bump the text there. (Unlike How to Play we can't just scale
    // the whole card up - that would break the volume slider's pointer math - so sizes are bumped
    // per element instead.) `port` gates every mobile size bump below.
    const port = H >= W;
    ...
    const block = scene.add.rectangle(CX, cardY + cardH / 2, cardW, cardH, 0x000000, 0.001).setInteractive();
    cont.add(block);
    cont.add(scene.add.text(CX, cardY + 46, 'Settings', { fontFamily: HEAD, fontSize: '40px', fontStyle: 'bold', color: INK }).setOrigin(0.5));
    makeClose(scene, cont, cardX + cardW - 40, cardY + 40, hide);

    const lx = cardX + 40;                 // left text column
    const rx = cardX + cardW - 40;         // right controls column
    const label = (y, s, size = port ? '26px' : '22px', color = INK) =>
    ...
    const dlssTxt = scene.add.text(lx, cardY + 380, 'DLSS + Frame Gen',
      { fontFamily: FONT, fontSize: port ? '26px' : '22px', fontStyle: 'bold', color: INK }).setOrigin(0, 0.5);
    cont.add(dlssTxt);
    ...
    const helpGap = 16, helpBtnW = (cardW - 80 - helpGap) / 2;
    makeButton(scene, cont, CX - helpGap / 2 - helpBtnW / 2, cardY + 490, helpBtnW, 64, 'How to Play', 0x39405c, INK, () => {
      hide();
      if (onHowToPlay) onHowToPlay();
    });
    makeButton(scene, cont, CX + helpGap / 2 + helpBtnW / 2, cardY + 490, helpBtnW, 64, 'Feedback', 0x39405c, INK, () => {
      hide();
      feedback.show();
    });
    ...
    cont.add(scene.add.text(CX, dy + 40, `Foothold  ${VERSION}`, { fontFamily: FONT, fontSize: port ? '24px' : '20px', fontStyle: 'bold', color: INK }).setOrigin(0.5));
    cont.add(scene.add.text(CX, dy + 72, 'A Benzur micro game · Icons by Kenney (CC0)',
      { fontFamily: FONT, fontSize: port ? '17px' : '14px', color: '#7c8398' }).setOrigin(0.5));
