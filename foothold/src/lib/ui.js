...
// ------------------------------------------------------------------ Feedback
// A tiny bug/feature report form, reachable from Settings ▸ Feedback (see createSettingsOverlay
// below). Uses real HTML <select>/<textarea> via Phaser's DOM element container (main.js enables
// `dom: { createContainer: true }`) rather than faking a dropdown/multiline input in canvas -
// Phaser has no native equivalent and a hand-rolled one would be worse UX for typing. Submits to
// the Cloudflare Worker in feedback.js, which turns it into a Trello card for triage.
export function createFeedbackOverlay(scene) {
  ...
  const show = () => {
    ...
    selectEl.className = 'fh-select';
    selectEl.style.width = `${fieldW}px`;
    selectEl.style.height = '48px';
    selectEl.style.fontSize = port ? '22px' : '18px';
    ['Bug', 'Feature'].forEach((v) => {
      const opt = document.createElement('option');
      opt.value = v; opt.textContent = v;
      selectEl.appendChild(opt);
    });
    const selectDom = scene.add.dom(CX, cardY + 196, selectEl).setOrigin(0.5);
    cont.add(selectDom);
    ...
    const submitBtn = makeButton(scene, cont, CX + gap / 2 + btnW / 2, btnY, btnW, 64, 'Submit', ACCENT, '#ffffff', async () => {
      ...
      const result = await submitFeedback({ type: selectEl.value, message, hp: hpEl.value });
      submitting = false;
